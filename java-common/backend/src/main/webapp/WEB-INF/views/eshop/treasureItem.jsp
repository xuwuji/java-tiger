				...
				<div class="modal-body">
					<div class="form-group">
						<label for="txt_departmentname">商品id</label> <input type="text"
							...
							class="form-control" id="add-productId" placeholder="商品id">
					</div>
					<div class="form-group">
						<label for="txt_departmentname">规格id</label> <input type="text"
							name="txt_departmentname" data-bind="value:Name"
							class="form-control" id="add-formatId" placeholder="规格id">
					</div>
					<div class="form-group">
						<label for="txt_departmentname">总人次</label> <input type="text"
							name="txt_departmentname" data-bind="value:Name"
							class="form-control" id="add-totalCount" placeholder="总人次">
					</div>
	...
	<div class="modal fade" id="editModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<label for="txt_departmentname">夺宝id</label> <input type="text"
				...
				<div class="form-group">
					<label for="txt_departmentname">规格id</label> <input type="text"
						name="txt_departmentname" data-bind="value:Name"
						class="form-control" id="edit-formatId" placeholder="商品id">
				</div>
				<div class="form-group">
					<label for="txt_departmentname">总人次</label> <input type="text"
						name="txt_departmentname" data-bind="value:Name"
						class="form-control" id="edit-totalCount" placeholder="总人次">
				</div>
				<div class="form-group">
					<label for="txt_departmentname">现在人次</label> <input type="text"
						name="txt_departmentname" data-bind="value:Name"
						class="form-control" id="edit-currentCount" placeholder="总人次">
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">
						<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>关闭
...
<script>
	...
	$table.bootstrapTable({
		url : '/backend/treasureItem/getAll', //请求后台的URL（*）
		method : 'get', //请求方式（*）
	...
	$('#btn_submit').on("click", function() {
		var productId = $('#add-productId').val();
		var totalCount = $('#add-totalCount').val();
		var formatId = $('#add-formatId').val();
		$.ajax({
			url : "/backend/treasureItem/add",
			type : "get",
			data : {
				productId : productId,
	...
	function edit(id) {
		...
		$("#editModal").modal().on("shown.bs.modal", function() {
			...
			$('#edit-totalCount').val(row.totalCount);
			$('#edit-currentCount').val(row.currentCount);
			$('#edit-formatId').val(row.formatId);
		});
	}
	...
	$('#btn_edit_submit').on("click", function() {
		...
		var totalCount = $('#edit-totalCount').val();
		var currentCount = $('#edit-currentCount').val();
		var formatId = $('#edit-formatId').val();
		$.ajax({
			url : "/backend/treasureItem/update",
			type : "get",
			data : {
				id : id,
				productId : productId,
	...
	function disable(id) {
		if (confirm("确定下架此分类吗？")) {
			$.ajax({
				url : "/backend/treasureItem/disable?id=" + id, //url
				type : "get",
				success : function(status) {
					$table.bootstrapTable('refresh');
				}
	...
	function reActive(id) {
		if (confirm("确定上架此分类吗？")) {
			$.ajax({
				url : "/backend/treasureItem/reActive?id=" + id, //url
				type : "get",
				success : function(status) {
					$table.bootstrapTable('refresh');
				}
