...
.jssora05l, .jssora05r {
	...
	cursor: pointer;
	background:
		url('${pageContext.request.contextPath}/resources/img/newsIndex/a17.png')
		no-repeat;
	overflow: hidden;
}
...
<script type="text/javascript">
	...
	function getDailyHotNews(tag, type) {
		...
		$
				.ajax(
						{
							type : "GET",
							url : "http://localhost:8080/backend/index/hotnews?time=&type="
									+ type,
	...
	function fixSilder(data) {
		...
		$
				.each(
						data,
						function(index, news) {
							str = '<div data - p = "112.50" style = "display: none;" >'
									+ '<a href=\'' + news.link + '\'>'
									+ '<img data - u = "image" src =\'' + news.imgLink + '\'' + ' /></a>'
									+ ' <div data - u = "thumb" > <a href =\'' + news.link + '\'>'
									+ news.title + '</a></div ></div > ' + str;
						});
	...
	function fixNewsContent(data) {
		...
		$.each(data, function(index, news) {
			str = '<li> <a href=\'' + news.link + '\'>' + news.title
					+ '</a></li>' + str;
			...
			url : "http://localhost:8080/backend/index/type",
				...
				function(data) {
					$.each(data, function(index, type) {
						typePart = typePart
								+ '<li onclick=getDailyHotNews(1,\'' + type
								+ '\')> ' + type + '</li>'
						console.log(type);
					});
